...
const input = {};
input.puzzle = `497,29 -> 497,27 -> 497,29 -> 499,29 -> 499,25 -> 499,29 -> 501,29 -> 501,20 -> 501,29 -> 503,29 -> 503,19 -> 503,29 -> 505,29 -> 505,25 -> 505,29 -> 507,29 -> 507,24 -> 507,29
...
function SandSimulator() {
  ...
  const parseInput = (data) => {
    ...
    lines.forEach(pathsRaw => {
      coordSets.push(pathsRaw.split(' -> ').map(coord => coord.split(',').reduce((acc, val, index) => {
        if (index === 0) {
          acc.x = parseInt(val);
        } else {
          acc.y = parseInt(val);
  ...
  function initGrid() {
    ...
    coordSets.forEach(coordSet => {
      for (let i = 0; i < coordSet.length-1; i++) {
        const curr = coordSet[i];
        const next = coordSet[i+1];
        let min, max;

        if (curr.x !== next.x && curr.y !== next.y) {
          console.log('WARNING: DIAGONAL LINES')
        } else if (curr.x !== next.x) {
          min = Math.min(curr.x, next.x);
          max = Math.max(curr.x, next.x);
          for (let j = min; j <= max; j++) {
            putItem(j, curr.y, '#');
          }
        } else if (curr.y !== next.y) {
          min = Math.min(curr.y, next.y);
          max = Math.max(curr.y, next.y);
          for (let j = min; j <= max; j++) {
            putItem(curr.x, j, '#');
          }
        } else {
          console.log('WARNING: LINE IS EQUAL');
        }
      };
    });
  ...
  function dropSand(x = 500, y = 1) {
    while (y !== floor && (!grid[y] || !grid[y][x]) ) {
      y++;
    }
...
console.log(`Part 2: ${ss.getTotal()}`)
