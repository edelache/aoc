...
const input = `dac abcf ac fdbcga dgcbae gcbfde fgcbd agfed adcgf cdbgfea | cbfa bcafdg cbfa bafcgd
...
const data = input.split('\n').map(line => {
  const [signalStr, valueStr] = line.split(' | ');
  const digits = signalStr.split(' ').map(str => str.split('').sort().join(''));
  const values = valueStr.split(' ').map(str => str.split('').sort().join(''));
  ...
  return {
    digits: digits.sort((a, b) => a.length > b.length ? 1 : -1),
    values
  }
});
...
for (let i = 0; i < 10; i++) {
  for (let j = 0; j < 10; j++) {
    ...
    let match = true;
    for (let k = 0; k < 7; k++) {
      if (digits[i][k] && digits[i][k] !== digits[j][k]) {
        match = false;
        break;
      }
    }
    if (match) {
      digitObjs[i][j] = digitObjs[j];
    }
  }
}
...
function determineKnown(digits) {
  const known = {};
  known[1] = digits[0]; // length 2
  known[4] = digits[2]; // length 4
  known[7] = digits[1]; // length 3
  known[8] = digits[9]; // length 7

  // 4 can be in only 9 for the 6 length objects, so we can figure out 9
  const sixLengthTaken = {};
  const fiveLengthTaken = {};

  for (let i = 6; i < 9; i++) {
    if (contains(known[4], digits[i])) {
      known[9] = digits[i];
      sixLengthTaken[i] = 1;
      break;
    }
  ...
  for (let i = 6; i < 9; i++) {
    if (sixLengthTaken[i]) continue;

    if (contains(known[7], digits[i])) {
      known[0] = digits[i];
      sixLengthTaken[i] = 1;
      break;
    }
  ...
  // 6 will be remaining option in 6 segment digits
  for (let i = 6; i < 9; i++) {
    if (sixLengthTaken[i]) continue;
  ...
  // now we can find 3, as only 9 and 8 contain 3 and whe know both of those
  for (let i = 3; i < 6; i++) {
    if (fiveLengthTaken[i]) continue;

    if (contains(digits[i], known[9])) {
      known[3] = digits[i];
      fiveLengthTaken[i] = 1;
      break;
    }
  ...
  // 2 will be remaining option in 5 segment digits
  for (let i = 3; i < 6; i++) {
    if (fiveLengthTaken[i]) continue;
...
data.forEach(d => {
  const known = determineKnown(d.digits);
  const lookup = {};
  Object.keys(known).forEach((key) => lookup[known[key]] = key);

  const clock = d.values.reduce((acc, val) => {
    acc += lookup[val]
    return acc;
  }, '')
  console.log(clock)
  sum += parseInt(clock, 10);
});
